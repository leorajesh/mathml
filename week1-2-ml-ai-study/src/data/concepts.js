import { calculusConcepts } from './calculusConcepts.js';
import { mlConcepts } from './mlConcepts.js';
import { mmlConcepts } from './mmlConcepts.js';
import { probabilityConcepts } from './probabilityConcepts.js';
import { subtopicConcepts, topics } from './subtopics.js';

// Reading order of every concept page (topics are overview pages, not listed here).
export const conceptOrder = [
  'ml-landscape',
  'ml-workflow',
  'feature-representation',
  'sets',
  'functions',
  'inverse-composition',
  'exp-log',
  'vectors-dot-product',
  'norms',
  'matrix-operations',
  'matrix-multiplication-outer-product',
  'matrix-systems',
  'gaussian-elimination',
  'solution-structure',
  'vector-spaces',
  'span-linear-combinations',
  'linear-independence',
  'subspaces',
  'basis-coordinates',
  'dimension',
  'linear-transformations',
  'transformation-matrix',
  'composition-of-transformations',
  'change-of-basis',
  'rank-nullity',
  'determinant-geometry',
  'determinants-cofactor-row-ops',
  'invertible-transformations',
  'affine-maps',
  'inner-products',
  'orthogonality',
  'orthogonal-complement',
  'orthogonal-projections',
  'gram-schmidt',
  'linear-classifier',
  'linear-classifier-through-origin',
  'linear-separability',
  'perceptron',
  'perceptron-convergence',
  'empirical-risk-zero-one',
  'hinge-loss',
  'max-margin-svm',
  'convex-functions',
  'surrogate-losses',
  'gradient-descent-method',
  'momentum',
  'subgradients',
  'stochastic-subgradient-descent',
  'lagrange-multipliers',
  'linear-regression',
  'polynomial-regression',
  'least-squares-normal-equation',
  'feature-scaling',
  'multicollinearity',
  'ridge-regularization',
  'lasso',
  'elastic-net',
  'model-complexity-generalization',
  'bias-variance',
  'train-validation-test',
  'cross-validation',
  'bootstrap',
  'logistic-regression',
  'logistic-loss',
  'classification-metrics',
  'roc-auc',
  'ml-in-production',
  'eigenvalues-eigenvectors',
  'trace',
  'diagonalization',
  'pagerank',
  'dimensionality-reduction',
  'spectral-theorem',
  'lu-decomposition',
  'cholesky-decomposition',
  'svd',
  'derivatives',
  'partial-derivatives-gradient',
  'jacobian-chain-rule',
  'loss-gradients',
  'backpropagation',
  'taylor-hessian',
  'probability-basics',
  'expectation-variance',
  'covariance-gaussian',
  'likelihood-mle',
  'pca',
];

export const sourceNotes = [
  'Mathematics for AI Week 0: Sets, Functions, Inverses, Composition',
  'Production ML Week 1: Introduction, Perceptron, Hinge Loss',
  'Production ML Week 2: Linear Regression, Ridge Regression, Logistic Regression',
  'Mathematics for AI Week 1: Matrices, Linear Systems, Vector Spaces, Bases, Linear Transformations',
  'Mathematics for AI Week 2: Invertible Matrices, Rank/Nullity, Affine Spaces, Determinants, Change of Basis',
  'Mathematics for AI Week 3: Eigenvalues, Eigenvectors, Diagonalization, PageRank, Spectral Theorem, Cholesky, LU, SVD',
  'Mathematics for Machine Learning (Deisenroth, Faisal, Ong): Ch. 3 analytic geometry, §4.1 trace, Ch. 5 vector calculus, Ch. 6 probability (§6.1-6.5), §7.1-7.2 momentum and Lagrange multipliers, Ch. 10 PCA',
];

export const notCovered = [
  'Administrative course logistics and syllabus items were omitted because they do not teach the requested ML or mathematics concepts.',
  'Later Production ML topics such as kernel SVMs (only the linear max-margin idea is covered, as an extension), clustering, decision trees, ensemble methods, HMMs, reinforcement learning, and anomaly detection were listed in the introduction slides but fall outside Week 1-2 coverage.',
  'The Mathematics for Machine Learning book is covered where it matches the course (Chapters 2-5, the probability basics of Chapter 6, §8.3 maximum likelihood, 7, and 10, plus least squares as a projection); Bayesian inference and MAP, Bayesian regression, Gaussian mixtures, and the book\'s SVM chapter are not converted (the max-margin page draws on Bishop and ISL instead).',
  'Long theorem proofs were compressed into intuition, formulas, and numeric examples so the app stays focused on conceptual understanding.',
];

const commonSymbols = {
  trainingSet: 'training examples indexed by t = 1, ..., n',
  x: 'feature vector describing one example',
  y: 'label (+1 or -1 in classification) or numeric response (in regression)',
  theta: 'parameter or weight vector learned from data',
};

const tex = String.raw;

const baseConcepts = [
  {
    id: 'ml-workflow',
    title: 'Supervised ML Workflow',
    group: 'Problem',
    week: 'Production ML W1',
    problem: 'It turns a vague goal like \'spot spam emails\' into a clear recipe a computer can follow and improve on.',
    intuition: 'Think of training a new employee to sort mail. You decide what they look at, what answers they give, what rules they may use, how you score their mistakes, and how they practise. The real test is tomorrow\'s mail, not the practice pile.',
    formulas: [
      {
        tex: tex`S_n = \{(x^{(t)}, y^{(t)})\}_{t=1}^{n}`,
        definitions: ['S_n: training set', 'n: number of training examples', 'x^{(t)}: encoded input for example t', 'y^{(t)}: observed output for example t'],
      },
      {
        tex: tex`\hat{h} = \arg\min_{h \in \mathcal{H}} E_n(h)`,
        definitions: ['\\mathcal{H}: hypothesis class or model family', 'E_n(h): training error or empirical risk', '\\hat{h}: selected predictor'],
      },
    ],
    example: [
      'Suppose three emails have labels spam, not spam, spam. Encode spam = +1 and not spam = -1.',
      'Encode each email with x = [contains_free, link_count], giving x^(1) = [1, 2], x^(2) = [0, 0], x^(3) = [1, 1].',
      'Choose a linear classifier h(x) = sign(theta dot x + theta_0). If theta = [2, 1] and theta_0 = -2, the scores are 2, -2, and 1.',
      'The signs are +, -, +, so all 3 training predictions match and E_n = 0 / 3 = 0.',
    ],
    graph: { type: 'workflow', title: 'Move along the ML workflow', sliders: [{ key: 'stage', label: 'Stage', min: 0, max: 5, step: 1, value: 0 }] },
    misconception: 'A high training score is not the final goal; the notes emphasize that the classifier should predict labels for new examples, so generalization is the real target.',
    prerequisites: ['ml-landscape'],
    followOns: ['feature-representation', 'linear-classifier', 'linear-regression', 'model-complexity-generalization', 'ml-in-production'],
    sources: ['Week1_01-Introduction.pdf', 'Production ML slides - Introduction Lesson1.pdf'],
  },
  {
    id: 'matrix-operations',
    title: 'Matrices and Basic Operations',
    group: 'Representation',
    week: 'Math W1C1',
    problem: 'They let us store a whole table of numbers, like a dataset or an image, and work on it all at once.',
    intuition: 'A matrix is a grid of numbers, like a spreadsheet, where the shape (rows by columns) matters. It lets us store a whole dataset or image and work on all of it at once.',
    formulas: [
      { tex: tex`A=\begin{bmatrix}a_{11}&\cdots&a_{1n}\\ \vdots& &\vdots\\ a_{m1}&\cdots&a_{mn}\end{bmatrix}\in\mathbb{R}^{m\times n}`, definitions: [tex`A: matrix`, tex`a_{ij}: entry in row i and column j`, tex`m: number of rows`, tex`n: number of columns`] },
      { tex: tex`(A+B)_{ij}=a_{ij}+b_{ij},\quad (cA)_{ij}=ca_{ij},\quad (A^T)_{ji}=a_{ij}`, definitions: [tex`B: second matrix with the same shape as A`, tex`c: scalar`, tex`A^T: transpose of A`] },
    ],
    example: ['Let A = [[1,2],[3,4]].', 'Scale entrywise: 2A = [[2,4],[6,8]].', 'Transpose: A^T = [[1,3],[2,4]].', 'Add A + A^T = [[2,5],[5,8]], which is symmetric.'],
    graph: { type: 'determinant', title: 'Matrix columns as area-shaping directions', sliders: [{ key: 'a', label: 'a', min: -2, max: 3, step: 0.25, value: 1 }, { key: 'b', label: 'b', min: -2, max: 3, step: 0.25, value: 2 }, { key: 'c', label: 'c', min: -2, max: 3, step: 0.25, value: 3 }, { key: 'd', label: 'd', min: -2, max: 3, step: 0.25, value: 4 }] },
    misconception: 'Two matrices cannot be added just because they contain the same number of entries; addition is entrywise, so the row and column shapes must match exactly.',
    prerequisites: ['vectors-dot-product'],
    followOns: ['matrix-multiplication-outer-product', 'matrix-systems'],
    sources: ['W1C1.pdf'],
  },
  {
    id: 'matrix-multiplication-outer-product',
    title: 'Matrix Multiplication and Outer Products',
    group: 'Representation',
    week: 'Math W1C1',
    problem: 'It performs many dot products in one go, and it is how we chain one transformation after another.',
    intuition: 'Matrix multiplication is many dot products done at once. It is also how one transformation is chained after another.',
    formulas: [
      { tex: tex`(AB)_{ij}=\sum_{\ell=1}^{k}a_{i\ell}b_{\ell j}`, definitions: [tex`A: m by k matrix`, tex`B: k by n matrix`, tex`k: shared inner dimension`, tex`(AB)_{ij}: row-column dot product`] },
      { tex: tex`a b^T=\begin{bmatrix}a_1b_1&\cdots&a_1b_n\\ \vdots& &\vdots\\ a_mb_1&\cdots&a_mb_n\end{bmatrix}`, definitions: [tex`a: column vector`, tex`b^T: row vector`, tex`ab^T: outer product, a rank-one matrix when a and b are nonzero`] },
      { tex: tex`(m\times k)(k\times n)=m\times n,\qquad (AB)C=A(BC),\qquad A(B+C)=AB+AC,\qquad (AB)^T=B^TA^T`, definitions: ['size rule: the inner sizes must match; a chain ABC is defined only when every neighbouring pair matches', 'order: AB and BA are generally different, and one may be defined when the other is not'] },
    ],
    example: ['Let A = [[1,2],[-2,1]] and B = [[3,5],[-1,4]].', 'For AB, the first row is [1*3 + 2*(-1), 1*5 + 2*4] = [1,13].', 'The second row is [-2*3 + 1*(-1), -2*5 + 1*4] = [-7,-6].', 'So AB = [[1,13],[-7,-6]], and reversing the order gives a different matrix.', 'Sizes decide what is defined: with P 2 by 3 and Q 3 by 3, PQ is 2 by 3, QP is not defined (3 is not 2), and P^T Q^T is not defined either, while Q P^T is 3 by 2.', 'A related fact (see the Trace page): even when AB and BA have different sizes, their traces agree. For A = [[1,0,2],[0,1,1]] (2 by 3) and B = [[1,1],[0,2],[1,0]] (3 by 2), AB = [[3,1],[1,2]] has trace 5 and BA = [[1,1,3],[0,2,2],[1,0,2]] has trace 5.'],
    graph: { type: 'outerProduct', title: 'Outer product as a rank-one building block', caption: 'Change entries of a and b. Every row of the product is a scaled copy of b^T, which is why one outer product contains just one independent direction.', sliders: [{ key: 'a1', label: 'a_1', min: -3, max: 3, step: 0.5, value: 1 }, { key: 'a2', label: 'a_2', min: -3, max: 3, step: 0.5, value: 2 }, { key: 'a3', label: 'a_3', min: -3, max: 3, step: 0.5, value: -1 }, { key: 'b1', label: 'b_1', min: -3, max: 3, step: 0.5, value: 2 }, { key: 'b2', label: 'b_2', min: -3, max: 3, step: 0.5, value: 1 }] },
    misconception: 'Matrix multiplication is not entrywise multiplication; it is defined to make matrix products match composition of linear maps.',
    prerequisites: ['matrix-operations'],
    followOns: ['matrix-systems', 'linear-transformations', 'least-squares-normal-equation'],
    sources: ['W1C1.pdf'],
  },
  {
    id: 'matrix-systems',
    title: 'Matrices and Linear Systems',
    group: 'Representation',
    week: 'Math W1C1',
    problem: 'It lets us write many linear equations as one compact statement, Ax = b, and solve them all together.',
    intuition: 'Picture a machine A that mixes the ingredients in x to produce a result b. Solving Ax = b asks: which ingredients give exactly this result?',
    formulas: [
      { tex: 'A x = b', definitions: ['A: coefficient matrix', 'x: unknown vector', 'b: right-hand-side vector'] },
      { tex: tex`R_i \leftarrow R_i + cR_j\quad (i\ne j)`, definitions: ['R_i: row i of a matrix', 'c: scalar multiplier', 'elementary row operation that preserves solutions (the rows must be different)'] },
    ],
    example: ['Solve 2a + b = 5 and a - b = 1.', 'Add the equations: 3a = 6, so a = 2.', 'Substitute into a - b = 1: 2 - b = 1, so b = 1.', 'Matrix form: [[2, 1], [1, -1]][a, b]^T = [5, 1]^T.'],
    graph: { type: 'linearSystem', title: 'Intersection of two linear equations', sliders: [{ key: 'm1', label: 'Line 1 slope', min: -3, max: 3, step: 0.25, value: -2 }, { key: 'b1', label: 'Line 1 intercept', min: -5, max: 5, step: 0.25, value: 5 }, { key: 'm2', label: 'Line 2 slope', min: -3, max: 3, step: 0.25, value: 1 }, { key: 'b2', label: 'Line 2 intercept', min: -5, max: 5, step: 0.25, value: -1 }] },
    figure: 'row-column-picture',
    misconception: 'Gaussian elimination is not a numerical trick separate from algebra; it is a sequence of equivalent equation systems.',
    prerequisites: ['matrix-multiplication-outer-product'],
    followOns: ['gaussian-elimination', 'span-linear-combinations', 'least-squares-normal-equation'],
    sources: ['W1C1.pdf'],
  },
  {
    id: 'gaussian-elimination',
    title: 'Gaussian Elimination, REF, and RREF',
    group: 'Optimization',
    week: 'Math W1C1',
    problem: 'It is a step-by-step recipe that simplifies any system of linear equations so you can read off whether it has a solution, and what it is.',
    intuition: 'A step-by-step recipe that turns a tangle of linear equations into a staircase you can read the answer from.',
    formulas: [
      { tex: tex`R_i\leftrightarrow R_j,\quad R_i\leftarrow cR_i\ (c\ne0),\quad R_i\leftarrow R_i+cR_j\ (i\ne j)`, definitions: [tex`R_i: row i`, tex`c: scalar multiplier`, tex`\leftrightarrow: row swap`] },
      { tex: tex`\begin{bmatrix}p_1&*&*\\0&p_2&*\\0&0&p_3\end{bmatrix}`, definitions: [tex`p_k: pivot`, tex`*: any entry`, tex`\text{RREF}: pivots are 1 and cleared above and below`] },
    ],
    example: ['Solve x + 2y + z = 4, 2x + y + z = 5, x + y + 2z = 5.', 'Subtract 2 row 1 from row 2 and row 1 from row 3 to clear the first column.', 'After eliminating column 2, the last row gives (4/3)z = 2, so z = 3/2 (this staircase is REF).', 'Back-substitution gives y = 1/2 and x = 3/2.', 'Going on to RREF instead (scale each pivot to 1 and clear above it) leaves the identity on the left, so the answer can be read off the last column.'],
    graph: { type: 'rowOpLines', title: 'Why row operations preserve solutions', caption: 'The orange line is row 2 plus k times row 1. It moves, but it keeps passing through the same solution point, so the system stays equivalent.', sliders: [{ key: 'k', label: 'row multiplier k', min: -3, max: 3, step: 0.25, value: 0 }] },
    misconception: 'Different elimination paths can produce different REF matrices, but the pivot columns, rank, and final RREF conclusions agree.',
    prerequisites: ['matrix-systems'],
    followOns: ['solution-structure', 'linear-independence', 'rank-nullity'],
    sources: ['W1C1.pdf'],
  },
  {
    id: 'solution-structure',
    title: 'Existence, Uniqueness, and Solution Sets',
    group: 'Optimization',
    week: 'Math W1C1-W2C1',
    problem: 'It tells you whether a system has no answer, exactly one answer, or infinitely many, before you trust any answer you compute.',
    intuition: 'Before trusting any answer, ask how many answers there are: none, exactly one, or infinitely many. After elimination, the pivots tell you.',
    formulas: [
      { tex: tex`\{x:Ax=b\}=\{x_p+x_h:Ax_h=0\}`, definitions: [tex`x_p: particular solution`, tex`x_h: homogeneous solution`, tex`\{x_h\}: null space`] },
      { tex: tex`\operatorname{rank}(A)+\operatorname{nullity}(A)=n`, definitions: [tex`\operatorname{rank}(A): pivot directions`, tex`\operatorname{nullity}(A): free directions`, tex`n: number of columns`] },
    ],
    example: ['Consider x1 + 2x2 - x3 = 3 and 2x1 + 4x2 - 2x3 = 6.', 'The second row is twice the first, so elimination leaves one nonzero row.', 'Let x2 = s and x3 = t. Then x1 = 3 - 2s + t.', 'The solution is [3,0,0]^T + s[-2,1,0]^T + t[1,0,1]^T.'],
    graph: { type: 'basis', title: 'Free parameters slide along null-space directions', caption: 'This 2D picture shows the homogeneous part s d_1 + t d_2. The full solution set adds the fixed particular solution x_p to every such point.', sliders: [{ key: 'c1', label: 'free parameter s', min: -4, max: 4, step: 0.25, value: 1 }, { key: 'c2', label: 'free parameter t', min: -4, max: 4, step: 0.25, value: 1 }, { key: 'tilt', label: 'direction tilt', min: -2, max: 2, step: 0.25, value: 0.5 }] },
    misconception: 'Infinitely many solutions are not unstructured; they form an affine set whose directions are exactly the null-space directions.',
    prerequisites: ['gaussian-elimination'],
    followOns: ['subspaces', 'ridge-regularization'],
    sources: ['W1C1.pdf', 'W2C1.pdf'],
  },
  {
    id: 'linear-transformations',
    title: 'Linear Transformations',
    group: 'Representation',
    week: 'Math W1C2 + W2C2',
    problem: 'They describe the "fair" ways a matrix can reshape space, stretching, rotating, reflecting, or shearing, and what such a map keeps or destroys.',
    intuition: 'A linear transformation reshapes space fairly: straight lines stay straight, the origin stays put, and combinations of inputs give the same combinations of outputs.',
    formulas: [
      { tex: 'T(av + bw) = aT(v) + bT(w)', definitions: ['T: transformation', 'a,b: scalars', 'v,w: vectors'] },
      { tex: tex`T(0)=0`, definitions: [tex`0: the zero vector; a map that moves the origin is not linear`] },
      { tex: tex`\ker T=\{v: T(v)=0\},\qquad \operatorname{im}T=\{T(v)\}`, definitions: [tex`\ker T: kernel (null space), inputs sent to zero`, tex`\operatorname{im}T: image (range), every output T can produce`] },
    ],
    example: ['Let T(x, y) = (2x, y), which doubles the x-coordinate. Then T([1, 3]) = [2, 3].', 'Check linearity: T(a[x1,y1] + b[x2,y2]) = (2(ax1 + bx2), ay1 + by2) = aT([x1,y1]) + bT([x2,y2]).', 'S(x, y) = (x + 1, y) is not linear: S([0, 0]) = [1, 0] moves the origin (it is affine).', 'The kernel of T is only [0, 0] (2x = 0 and y = 0), and its image is all of R^2, so T loses no information.'],
    graph: { type: 'transform', title: 'Matrix transformation of a square', sliders: [{ key: 'scaleX', label: 'x scale', min: 0.3, max: 2.5, step: 0.1, value: 1.4 }, { key: 'shear', label: 'shear', min: -1.5, max: 1.5, step: 0.1, value: 0.4 }, { key: 'scaleY', label: 'y scale', min: 0.3, max: 2.5, step: 0.1, value: 0.9 }] },
    misconception: 'Not every map that keeps straight lines straight is linear: a shift keeps lines straight but moves the origin, so it is affine, not linear (see S in the example).',
    prerequisites: ['basis-coordinates'],
    followOns: ['transformation-matrix', 'affine-maps', 'rank-nullity', 'least-squares-normal-equation'],
    sources: ['W1C2.pdf', 'W2C2.pdf'],
  },
  {
    id: 'transformation-matrix',
    title: 'Transformation Matrix',
    group: 'Model',
    week: 'Math W1C2',
    problem: 'It turns a transformation described in words into a grid of numbers a computer can apply to any vector.',
    intuition: 'To turn a transformation into numbers, just track where the basic directions land, such as one step right and one step up.',
    formulas: [
      { tex: tex`[T]=\big[T(e_1)\;T(e_2)\;\cdots\;T(e_n)\big],\quad T(x)=[T]x`, definitions: [tex`T: linear transformation`, tex`e_j: j-th standard basis vector`, tex`[T]: transformation matrix`] },
      { tex: tex`[T]_{CB}=\big[[T(v_1)]_C\;[T(v_2)]_C\;\cdots\;[T(v_n)]_C\big]`, definitions: [tex`B=\{v_j\}: source basis`, tex`C: target basis`, tex`[\cdot]_C: coordinates in basis C`] },
      { tex: tex`T(x)=\begin{bmatrix}a_{11}x_1+\cdots+a_{1n}x_n\\ \vdots\\ a_{m1}x_1+\cdots+a_{mn}x_n\end{bmatrix}\ \Rightarrow\ [T]=\begin{bmatrix}a_{11}&\cdots&a_{1n}\\ \vdots&&\vdots\\ a_{m1}&\cdots&a_{mn}\end{bmatrix}`, definitions: [tex`a_{ij}: coefficient of x_j in output i; row i of the matrix lists the coefficients of output i, and a map from R^n to R^m has an m by n matrix`] },
      { tex: tex`D:P_2\to P_1,\ \ B=(x^2,x,1),\ C=(x,1):\qquad [D]_{CB}=\begin{bmatrix}2&0&0\\0&1&0\end{bmatrix}`, definitions: [tex`D: differentiation, a linear map between polynomial spaces; its columns are the C-coordinates of D(x^2) = 2x, D(x) = 1 and D(1) = 0`] },
    ],
    example: ['For T(x,y) = (x, -y), compute images of the standard basis.', 'T(e1) = T([1,0]) = [1,0].', 'T(e2) = T([0,1]) = [0,-1].', 'Stacking these as columns gives [T] = [[1,0],[0,-1]].', 'In three dimensions the same recipe works. Reflecting through the xz-plane flips only the y-coordinate: e1 -> e1, e2 -> -e2, e3 -> e3, so the matrix is [[1,0,0],[0,-1,0],[0,0,1]].', 'Projecting onto the yz-plane keeps y and z and sends x to 0: e1 -> 0, so the matrix is [[0,0,0],[0,1,0],[0,0,1]]. A projection has a zero column (its null space is the direction it squashes).', 'A map between polynomial spaces: differentiation D: P2 -> P1 is linear (the derivative of a sum is the sum of derivatives). With input basis (x^2, x, 1) and output basis (x, 1): D(x^2) = 2x = [2,0], D(x) = 1 = [0,1], D(1) = 0 = [0,0].', 'So [D] = [[2,0,0],[0,1,0]], a 2 by 3 matrix (dim P1 by dim P2). The order of each basis matters: with input (1, x, x^2) and output (x, 1) the matrix is [[0,0,2],[0,1,0]]; with both ascending, (1, x, x^2) and (1, x), it is [[0,1,0],[0,0,2]]. Always state both orders.', 'When only the input order is given, use the same pattern for the output: with descending powers, D: P3 -> P2 in the bases (x^3, x^2, x, 1) and (x^2, x, 1) has matrix [[3,0,0,0],[0,2,0,0],[0,0,1,0]], one column per input basis polynomial and a zero column for the constant.'],
    graph: { type: 'transform', title: 'Columns show where basis vectors go', sliders: [{ key: 'scaleX', label: 'image of e_1 scale', min: 0.3, max: 2.5, step: 0.1, value: 1 }, { key: 'shear', label: 'mixing/shear', min: -1.5, max: 1.5, step: 0.1, value: 0 }, { key: 'scaleY', label: 'image of e_2 scale', min: 0.3, max: 2.5, step: 0.1, value: 1 }] },
    misconception: 'A linear map does not need infinitely many examples to specify it; once a basis is fixed, its basis-vector images determine everything.',
    prerequisites: ['linear-transformations', 'basis-coordinates'],
    followOns: ['composition-of-transformations', 'change-of-basis', 'invertible-transformations'],
    sources: ['W1C2.pdf'],
  },
  {
    id: 'composition-of-transformations',
    title: 'Composition of Transformations',
    group: 'Model',
    week: 'Math W1C2',
    problem: 'It explains why matrix multiplication follows its row-times-column rule and why the order you apply steps in matters.',
    intuition: 'Doing one transformation and then another is like socks then shoes: the order changes the result.',
    formulas: [
      { tex: tex`(T_2\circ T_1)(u)=T_2(T_1(u)),\quad [T_2\circ T_1]=[T_2][T_1]`, definitions: [tex`T_1: first map applied`, tex`T_2: second map applied`, tex`\circ: composition`] },
      { tex: tex`(AB)_{ij}=\sum_{\ell=1}^{k}a_{i\ell}b_{\ell j}`, definitions: [tex`A: matrix for the second map`, tex`B: matrix for the first map`, tex`k: intermediate dimension`] },
    ],
    example: ['Let R rotate by 90 degrees and F reflect across the x-axis.', 'R = [[0,-1],[1,0]] and F = [[1,0],[0,-1]].', 'Rotate then reflect gives FR = [[0,-1],[-1,0]].', 'Reflect then rotate gives RF = [[0,1],[1,0]], so the order changes the transformation.'],
    graph: { type: 'transform', title: 'Composed maps change a square in stages', sliders: [{ key: 'scaleX', label: 'first scale', min: 0.3, max: 2.5, step: 0.1, value: 1.4 }, { key: 'shear', label: 'second shear', min: -1.5, max: 1.5, step: 0.1, value: 0.6 }, { key: 'scaleY', label: 'second scale', min: 0.3, max: 2.5, step: 0.1, value: 0.8 }] },
    misconception: 'Matrix multiplication is not arbitrary notation; it is the only product that correctly represents composition of linear maps.',
    prerequisites: ['transformation-matrix', 'matrix-multiplication-outer-product'],
    followOns: ['invertible-transformations', 'lu-decomposition'],
    sources: ['W1C2.pdf'],
  },
  {
    id: 'change-of-basis',
    title: 'Change of Basis',
    group: 'Representation',
    week: 'Math W2C2',
    problem: 'It converts the coordinates of a vector from one grid to another without moving the vector itself.',
    intuition: 'Change of basis rewrites a vector\'s coordinates in a new grid without moving the vector itself.',
    formulas: [
      { tex: tex`P=\big[b_1\;b_2\;\cdots\;b_n\big],\quad [v]_{\text{std}}=P[v]_B,\quad [v]_B=P^{-1}[v]_{\text{std}}`, definitions: [tex`P: change-of-basis matrix`, tex`b_i: i-th basis vector`, tex`[v]_B: coordinates in basis B`] },
      { tex: tex`[T]_B=P^{-1}AP`, definitions: [tex`A: matrix in standard coordinates`, tex`[T]_B: same transformation in basis B`, tex`P^{-1}AP: similarity transform`] },
      { tex: tex`[I]_{CB}=\big[[b_1]_C\;[b_2]_C\;\cdots\;[b_n]_C\big],\qquad [v]_C=[I]_{CB}\,[v]_B`, definitions: [tex`[I]_{CB}: change-of-basis matrix from B to C: its columns are the old basis vectors written in the new basis; the right subscript is the input basis, as in [T]_{CB}`, tex`[v]_C: coordinates of v in basis C`] },
      { tex: tex`[I]_{SB}=P_B,\qquad [I]_{BS}=P_B^{-1},\qquad [I]_{CB}=P_C^{-1}P_B`, definitions: [tex`S: the standard basis (written std elsewhere on this site)`, tex`P_B: the matrix whose columns are the basis vectors of B; so the P above is the change of basis from B to the standard basis`] },
      { tex: tex`[T]_{C'B'}=[I]_{C'C}\,[T]_{CB}\,[I]_{BB'}`, definitions: [tex`[T]_{C'B'}: the same map T described with new bases B' (input) and C' (output); read right to left: convert B'-coordinates to B, apply T, convert C-coordinates to C'`] },
    ],
    example: ['Use basis B = {[1,1], [1,-1]} and vector v = [3,1].', 'Solve alpha[1,1] + beta[1,-1] = [3,1].', 'The equations are alpha + beta = 3 and alpha - beta = 1.', 'Thus alpha = 2 and beta = 1, so [v]_B = [2,1].', 'Between two non-standard bases: take B = {[1,2], [1,-1]} and B~ = {[3,0], [0,3]}. The change of basis from B~ to B has columns [b~1]_B and [b~2]_B.', 'Solve a[1,2] + b[1,-1] = [3,0]: a + b = 3 and 2a - b = 0, so [b~1]_B = [1,2]. Solve a[1,2] + b[1,-1] = [0,3]: a + b = 0 and 2a - b = 3, so [b~2]_B = [1,-1].', 'So [I]_{BB~} = [[1,1],[2,-1]] (columns [1,2] and [1,-1]). Check with P_B^{-1} P_B~ = P_B^{-1}(3I): the same matrix.', 'A map given on combinations: suppose T(b1 + b2) = [4,1] and T(b1 - b2) = [2,-1]. Adding and halving gives T(b1) = [3,0]; subtracting and halving gives T(b2) = [1,1]. So [T]_{SB} = [[3,1],[0,1]].', 'Then [T]_{SB~} = [T]_{SB}[I]_{BB~} = [[3,1],[0,1]][[1,1],[2,-1]] = [[5,2],[2,-1]]. Check one column directly: b~1 = 1·b1 + 2·b2, so T(b~1) = [3,0] + 2[1,1] = [5,2].', 'If the output should use a non-standard basis C instead, convert the output: [T]_{CB} = [I]_{CS}[T]_{SB} = P_C^{-1}[T]_{SB}. Equivalently, write each image T(b_j) in C-coordinates (solve P_C y = T(b_j)) and use those as the columns.'],
    graph: { type: 'basisCoords', title: 'Same vector, different coordinate grid', caption: 'The orange vector is fixed in standard coordinates. The readout shows how its coordinates change when you describe it using the diagonal basis B.', sliders: [{ key: 'vx', label: 'standard x-coordinate', min: -5, max: 5, step: 0.25, value: 3 }, { key: 'vy', label: 'standard y-coordinate', min: -5, max: 5, step: 0.25, value: 1 }] },
    misconception: 'Changing basis changes the coordinate numbers, not the vector or the underlying transformation. Watch the direction: P = [b1 ... bn] converts B-coordinates into standard coordinates, so it is the change of basis FROM B TO the standard basis, [I]_{SB}; its inverse goes the other way.',
    prerequisites: ['basis-coordinates', 'transformation-matrix'],
    followOns: ['diagonalization', 'orthogonality'],
    sources: ['W2C2.pdf'],
  },
  {
    id: 'invertible-transformations',
    title: 'Invertible Transformations',
    group: 'Optimization',
    week: 'Math W1C2-W2C1',
    problem: 'It tells us when a transformation loses no information, so we can always undo it and recover the exact input.',
    intuition: 'A transformation can be undone when every output comes from exactly one input: nothing is squashed together, and every output is reached.',
    formulas: [
      { tex: tex`T^{-1}(T(v))=v,\quad T(T^{-1}(w))=w`, definitions: [tex`T: transformation`, tex`T^{-1}: inverse transformation`, tex`v,w: vectors in the source and target spaces`] },
      { tex: tex`AA^{-1}=A^{-1}A=I,\quad \det(A)\ne0`, definitions: [tex`A^{-1}: inverse matrix`, tex`I: identity matrix`, tex`\det(A): determinant`] },
      { tex: tex`[A\mid I]\ \xrightarrow{\text{row reduce}}\ [I\mid A^{-1}]`, definitions: [tex`[A\mid I]: augmented matrix`, tex`I: identity matrix`, tex`A^{-1}: inverse found by Gaussian elimination`] },
    ],
    example: ['Let A = [[3,5],[-1,4]].', 'det(A) = 3*4 - 5*(-1) = 17, so A is invertible.', 'The 2x2 inverse formula gives A^{-1} = (1/17)[[4,-5],[1,3]].', 'Multiplying A A^{-1} gives (1/17)[[17,0],[0,17]] = I.', 'Gauss-Jordan for a 3 by 3 matrix: A = [[0,1,1],[1,2,0],[0,1,2]]. Write [A | I] and swap R1 and R2, because the top-left entry is 0.', 'Clear column 2: R1 <- R1 - 2R2 gives [1,0,-2 | -2,1,0], and R3 <- R3 - R2 gives [0,0,1 | -1,0,1].', 'Clear column 3 upwards: R2 <- R2 - R3 gives [0,1,0 | 2,0,-1], and R1 <- R1 + 2R3 gives [1,0,0 | -4,1,2]. The left block is now I.', 'So A^{-1} = [[-4,1,2],[2,0,-1],[-1,0,1]]. Check one entry of A A^{-1}: row 1 of A times column 1 of A^{-1} = 0·(-4) + 1·2 + 1·(-1) = 1.'],
    graph: { type: 'determinant', title: 'Invertible means area does not collapse', sliders: [{ key: 'a', label: 'a', min: -3, max: 3, step: 0.25, value: 3 }, { key: 'b', label: 'b', min: -3, max: 3, step: 0.25, value: 1 }, { key: 'c', label: 'c', min: -3, max: 3, step: 0.25, value: -1 }, { key: 'd', label: 'd', min: -3, max: 3, step: 0.25, value: 2 }] },
    misconception: 'Solving Ax=b does not require explicitly forming A^{-1}; elimination and factorizations are usually faster and more stable.',
    prerequisites: ['composition-of-transformations', 'gaussian-elimination', 'determinant-geometry'],
    followOns: ['affine-maps', 'lu-decomposition'],
    sources: ['W1C2.pdf', 'W2C1.pdf'],
  },
  {
    id: 'determinants-cofactor-row-ops',
    title: 'Determinants: Cofactors and Row Operations',
    group: 'Optimization',
    week: 'Math W2C2',
    problem: 'They give practical ways to compute a determinant, the single number that tells you whether a square matrix can be undone.',
    intuition: 'The determinant says how much a matrix stretches area or volume, with a sign for flips. These are the practical ways to compute it.',
    formulas: [
      { tex: tex`\det(A)=\sum_{j=1}^{n}(-1)^{1+j}a_{1j}M_{1j}`, definitions: [tex`M_{1j}: minor determinant after deleting row 1 and column j`, tex`(-1)^{1+j}: cofactor sign`] },
      { tex: tex`R_i\leftrightarrow R_j:\det\mapsto-\det,\quad R_i\leftarrow cR_i:\det\mapsto c\det,\quad R_i\leftarrow R_i+cR_j\ (i\ne j):\det\mapsto\det`, definitions: [tex`R_i: row i`, tex`c: scalar`, tex`\det: determinant`] },
      { tex: tex`A^{-1}=\frac{1}{\det A}\operatorname{adj}(A),\qquad \operatorname{adj}(A)_{ij}=C_{ji}=(-1)^{i+j}M_{ji}`, definitions: [tex`C_{ji}: cofactor of entry (j, i), the signed minor; note the transpose`, tex`\operatorname{adj}(A): adjugate, the transposed matrix of cofactors; A adj(A) = det(A) I`] },
      { tex: tex`\det(U)=u_{11}u_{22}\cdots u_{nn}\ \text{(triangular)},\qquad \det(A)=(-1)^{s}\det(U)`, definitions: [tex`U: an upper-triangular matrix reached from A by row replacements R_i <- R_i + cR_j and s row swaps (if you scale a row by c, divide by c at the end)`] },
      { tex: tex`\det(A)=\sum_{j=1}^{n}(-1)^{i+j}a_{ij}M_{ij}\ \ \text{(any row }i),\qquad \det(A)=\sum_{i=1}^{n}(-1)^{i+j}a_{ij}M_{ij}\ \ \text{(any column }j)`, definitions: [tex`(-1)^{i+j}: sign checkerboard + - + ... starting with + in the top-left; expand along the row or column with the most zeros`] },
    ],
    example: ['Let A = [[3,2,2],[2,3,2],[2,2,3]].', 'Cofactors along row 1 give M11 = 5, M12 = 2, and M13 = -2.', 'det(A) = 3(5) - 2(2) + 2(-2) = 15 - 4 - 4 = 7.', 'Since det(A) is nonzero, A is invertible.', 'For bigger matrices, reduce to triangular form instead. Take A = [[1,2,0,1],[2,3,1,0],[0,1,2,3],[1,0,1,2]]: R2 - 2R1 and R4 - R1 give rows [0,-1,1,-2] and [0,-2,1,1]; row replacements do not change det.', 'Then R3 + R2 gives [0,0,3,1] and R4 - 2R2 gives [0,0,-1,5]. Swap R3 and R4 (one swap: det changes sign) and do R4 + 3R3: the last row becomes [0,0,0,16].', 'Now U = [[1,2,0,1],[0,-1,1,-2],[0,0,-1,5],[0,0,0,16]] is triangular: det U = 1·(-1)·(-1)·16 = 16, and one swap gives det A = -16.', 'You can also mix the two methods: use row replacements to create zeros in one column, then expand along it. In the same A, R3 - 2R2 and R4 - R2 turn column 3 into [0,1,0,0], so expanding along column 3 leaves one term: (-1)^(2+3)·1·det[[1,2,1],[-4,-5,3],[-1,-3,2]] = -16, as before.'],
    graph: { type: 'determinant', title: 'Row operations and determinant as area scaling', caption: 'The determinant is zero exactly when area collapses; row operations are useful because they can simplify the matrix without losing track of determinant changes.', sliders: [{ key: 'a', label: 'a', min: -3, max: 3, step: 0.25, value: 3 }, { key: 'b', label: 'b', min: -3, max: 3, step: 0.25, value: 2 }, { key: 'c', label: 'c', min: -3, max: 3, step: 0.25, value: 1 }, { key: 'd', label: 'd', min: -3, max: 3, step: 0.25, value: 3 }] },
    misconception: 'Row replacement does not change the determinant, but row swaps and row scaling do; mixing these rules is a common source of wrong determinant computations.',
    prerequisites: ['determinant-geometry', 'gaussian-elimination'],
    followOns: ['change-of-basis', 'eigenvalues-eigenvectors'],
    sources: ['W2C2.pdf'],
  },
  {
    id: 'linear-classifier',
    title: 'Linear Classifiers and Decision Boundaries',
    group: 'Model',
    week: 'Production ML W1',
    problem: 'It sorts examples into two groups by drawing a straight line (or flat plane) between them.',
    intuition: 'Draw a straight line between two groups of points: everything on one side is +1, everything on the other is -1. In more dimensions the line becomes a flat plane.',
    formulas: [
      { tex: tex`h(x;\theta,\theta_0)=\operatorname{sign}(\theta \cdot x + \theta_0)`, definitions: ['h: predicted class', 'theta: normal vector to the boundary', 'theta_0: intercept or bias', 'x: feature vector'] },
      { tex: tex`X^+(\theta,\theta_0)=\{x:\theta\cdot x+\theta_0\ge0\},\qquad X^-(\theta,\theta_0)=\{x:\theta\cdot x+\theta_0<0\}`, definitions: [tex`X^+,X^-: the two half-spaces the classifier predicts as +1 and -1, separated by the decision boundary`] },
      { tex: tex`\theta \cdot x + \theta_0 = 0`, definitions: ['decision boundary: all points whose score is exactly zero'] },
    ],
    example: ['Let theta = [1, 2] and theta_0 = -3.', 'For x = [1, 2], score = 1*1 + 2*2 - 3 = 2, so h(x) = +1.', 'For x = [1, 0], score = 1*1 + 2*0 - 3 = -2, so h(x) = -1.'],
    graph: { type: 'linearBoundary', title: 'Move the separating line', sliders: [{ key: 'theta1', label: 'theta_1', min: -4, max: 4, step: 0.25, value: 1 }, { key: 'theta2', label: 'theta_2', min: -4, max: 4, step: 0.25, value: 2 }, { key: 'bias', label: 'theta_0', min: -5, max: 5, step: 0.25, value: -1 }] },
    misconception: 'The weights are not the line itself; they are perpendicular to the decision boundary and define its orientation.',
    prerequisites: ['feature-representation', 'vectors-dot-product'],
    followOns: ['linear-classifier-through-origin', 'linear-separability', 'perceptron', 'empirical-risk-zero-one', 'logistic-regression'],
    sources: ['Week1_02-PerceptronLinearClassifier.pdf', 'Production ML slides - Linear Classification Lesson 2.pdf'],
  },
  {
    id: 'linear-classifier-through-origin',
    title: 'Linear Classifier Through Origin',
    group: 'Model',
    week: 'Production ML W1',
    problem: 'It is the simplest linear classifier: the dividing line must pass through the origin, before we add a bias term to let it move.',
    intuition: 'Picture a clock hand pinned at the centre: it can rotate to any angle but cannot move sideways. Without theta_0, the dividing line is pinned to the origin in the same way.',
    formulas: [
      { tex: tex`h(x;\theta)=\operatorname{sign}(\theta\cdot x)`, definitions: [tex`h: predicted class`, tex`\theta: weight vector`, tex`x: feature vector`] },
      { tex: tex`\theta\cdot x=0`, definitions: [tex`\theta: normal vector`, tex`\theta\cdot x: score`, 'decision boundary: a line (hyperplane) through the origin, perpendicular to theta'] },
    ],
    example: ['Let theta = [1, -1].', 'For x = [2, 1], theta dot x = 2 - 1 = 1, so predict +1.', 'For x = [1, 2], theta dot x = 1 - 2 = -1, so predict -1.', 'The boundary x1 - x2 = 0 is the line x1 = x2 and passes through the origin.'],
    graph: { type: 'linearBoundary', title: 'Through-origin boundary', caption: 'The bias is fixed at 0, so the line can rotate but cannot shift away from the origin. The arrow is theta; the shaded side is predicted +1.', sliders: [{ key: 'theta1', label: 'theta_1', min: -4, max: 4, step: 0.25, value: 1 }, { key: 'theta2', label: 'theta_2', min: -4, max: 4, step: 0.25, value: -1 }], fixed: { bias: 0 } },
    misconception: 'A through-origin classifier is not just a general classifier with theta_0 forgotten; it is a stricter hypothesis class that may fail on data separable with a shifted boundary.',
    prerequisites: ['linear-classifier'],
    followOns: ['linear-separability', 'perceptron'],
    sources: ['Week1_02-PerceptronLinearClassifier.pdf', 'Production ML slides - Linear Classification Lesson 2.pdf'],
  },
  {
    id: 'linear-separability',
    title: 'Linear Separability',
    group: 'Model',
    week: 'Production ML W1',
    problem: 'It tells us whether a perfect straight dividing line even exists, before we ask an algorithm to find one.',
    intuition: 'Before asking an algorithm to find a perfect dividing line, ask whether one exists. Data are linearly separable if some line puts every positive on one side and every negative on the other.',
    formulas: [
      { tex: tex`\exists\hat\theta:\quad y^{(t)}(\hat\theta\cdot x^{(t)})>0\quad\forall t`, definitions: ['separable through the origin: Definition 1.1 of the Week 1 notes; when this holds, the setting is called the realizable case (some classifier in the class makes zero training errors)'] },
      { tex: tex`\exists\theta,\theta_0:\quad y^{(t)}(\theta\cdot x^{(t)}+\theta_0)>0\quad\forall t`, definitions: [tex`\theta: normal vector`, tex`\theta_0: offset`, tex`y^{(t)}: label`] },
      { tex: tex`\gamma=\min_t \frac{y^{(t)}(\theta\cdot x^{(t)}+\theta_0)}{\lVert\theta\rVert}`, definitions: [tex`\gamma: geometric margin of a chosen separator (distance from the boundary to the closest point)`, tex`\lVert\theta\rVert: dividing by it makes gamma unchanged when theta and theta_0 are rescaled`, tex`t: training-example index`] },
    ],
    example: ['In one dimension, a negative point at x = 1 and a positive point at x = 3 are separable with boundary x = 2.', 'Choose score s(x) = x - 2.', 'For y = -1 at x = 1, y s(x) = (-1)(-1) = 1 > 0.', 'For y = +1 at x = 3, y s(x) = (1)(1) = 1 > 0. Both signed margins are positive, and since ||theta|| = 1 here the geometric margin is gamma = 1.'],
    graph: { type: 'linearBoundary', title: 'A line separates only when labels do not overlap', caption: 'This line separates the five points (0 mistakes). Now set the overlap slider to 1: an extra red point lands among the green ones, and no setting of the sliders can reach zero mistakes.', sliders: [{ key: 'theta1', label: 'theta_1', min: -4, max: 4, step: 0.25, value: -1 }, { key: 'theta2', label: 'theta_2', min: -4, max: 4, step: 0.25, value: 1 }, { key: 'bias', label: 'theta_0', min: -5, max: 5, step: 0.25, value: 0 }, { key: 'overlap', label: 'add overlapping point (0 = no, 1 = yes)', min: 0, max: 1, step: 1, value: 0 }] },
    misconception: 'Not linearly separable does not mean not learnable; it means this feature representation and linear hypothesis class cannot be perfect.',
    prerequisites: ['linear-classifier'],
    followOns: ['perceptron', 'hinge-loss'],
    sources: ['Week1_02-PerceptronLinearClassifier.pdf', 'Production ML slides - Linear Classification Lesson 2.pdf'],
  },
  {
    id: 'perceptron',
    title: 'Perceptron Algorithm',
    group: 'Optimization',
    week: 'Production ML W1',
    problem: 'It is a simple learning rule that finds a separating line whenever one exists.',
    intuition: 'Learn by fixing mistakes. Walk through the examples, and whenever the line gets one wrong, nudge it toward the right answer. Examples it already gets right change nothing.',
    formulas: [
      { tex: tex`\text{if } y^{(t)}(\theta \cdot x^{(t)} + \theta_0) \le 0,\quad \theta \leftarrow \theta + y^{(t)}x^{(t)}`, definitions: ['y^(t): true label, either +1 or -1', 'x^(t): current example', 'theta: weight vector before update'] },
      { tex: tex`\theta_0 \leftarrow \theta_0 + y^{(t)}`, definitions: ['theta_0: bias term updated in the general-case perceptron'] },
    ],
    example: ['Start theta = [0, 0], theta_0 = 0.', 'Example x = [2, 1], y = +1 gives score = 0, so y*score = 0 and it is treated as a mistake (the notes count a point on the boundary as an error, as in their training-error formula).', 'Update theta = [0,0] + 1*[2,1] = [2,1]; theta_0 = 0 + 1 = 1.', 'New score on the same point = [2,1] dot [2,1] + 1 = 4 + 1 + 1 = 6, now positive.'],
    graph: { type: 'perceptron', title: 'One perceptron correction (y = +1)', caption: 'The blue arrow is theta before the update, the orange arrow adds y x, and the red arrow is the new theta.', sliders: [{ key: 'theta1', label: 'theta_1 before', min: -3, max: 3, step: 0.25, value: -1 }, { key: 'theta2', label: 'theta_2 before', min: -3, max: 3, step: 0.25, value: 0.5 }, { key: 'x1', label: 'x_1', min: -3, max: 3, step: 0.25, value: 2 }, { key: 'x2', label: 'x_2', min: -3, max: 3, step: 0.25, value: 1 }] },
    misconception: 'Perceptron convergence requires linear separability; in the non-separable case it can keep cycling rather than settle on the best training-error classifier.',
    prerequisites: ['linear-classifier', 'linear-separability'],
    followOns: ['perceptron-convergence', 'hinge-loss', 'gradient-descent-method'],
    sources: ['Week1_02-PerceptronLinearClassifier.pdf', 'Production ML slides - Linear Classification Lesson 2.pdf'],
  },
  {
    id: 'perceptron-convergence',
    title: 'Perceptron Convergence and Failure Mode',
    group: 'Optimization',
    week: 'Production ML W1',
    problem: 'It explains why the perceptron is guaranteed to stop on separable data, and why it can loop forever when the classes overlap.',
    intuition: 'Why does the perceptron ever stop? Each correction makes real progress toward a good separator, and when there is a gap between the classes, there is only so much progress to make.',
    formulas: [
      { tex: tex`y^{(t)}(\theta^{(k+1)}\cdot x^{(t)})=y^{(t)}(\theta^{(k)}\cdot x^{(t)})+\lVert x^{(t)}\rVert^2`, definitions: [tex`k: update index`, tex`\lVert x^{(t)}\rVert^2: squared feature length (through-origin update; with an offset the gain is ||x||^2 + 1)`] },
      { tex: tex`\text{finite mistakes}\quad\Leftarrow\quad\text{linear separability}`, definitions: [tex`\text{finite mistakes}: algorithm eventually stops updating`] },
      { tex: tex`\lVert x^{(t)}\rVert\le R,\ \ y^{(t)}\frac{\theta^*\cdot x^{(t)}}{\lVert\theta^*\rVert}\ge\gamma>0\ \ \forall t\quad\Rightarrow\quad M\le\left(\frac{R}{\gamma}\right)^2`, definitions: [tex`M: number of perceptron mistakes`, tex`R: bound on every feature-vector norm`, tex`\theta^*: some separator through the origin`, tex`\gamma: geometric margin of that separator`, tex`\text{offset}: append a constant feature 1 to each x to handle theta_0`, 'start: the bound assumes theta^(0) = 0, as in the notes; with random initialization it changes'] },
    ],
    example: ['Suppose an example has signed agreement -0.5 before the update and ||x||^2 = 5.', 'After the update, agreement becomes -0.5 + 5 = 4.5.', 'That example is now correctly classified with positive margin, here because ||x||^2 = 5 exceeds the deficit 0.5; had the agreement been -10, one update would undershoot and the example would still be wrong.', 'The convergence theorem says this cannot require infinitely many mistakes when a separator exists.'],
    graph: { type: 'perceptronMistakes', title: 'Margin controls how many mistakes the perceptron makes', caption: 'Ten points separated by the dashed diagonal with margin gamma. The perceptron (through the origin, starting at theta = 0) cycles through them until a full pass has no mistakes. Shrink the margin and watch the mistake count and the bound (R/gamma)^2.', sliders: [{ key: 'gamma', label: 'margin gamma', min: 0.05, max: 1.5, step: 0.05, value: 0.4 }] },
    misconception: 'Converging does not mean converging quickly or to a unique answer. As the Lesson 2 slides note, the finite number of mistakes can still be large, there can be more than one separating solution (which one you get depends on the initialization and the order of examples), and if the data are not linearly separable, training never converges: more passes will not help, and the fix is a different loss (such as hinge) or better features.',
    prerequisites: ['perceptron', 'linear-separability'],
    followOns: ['hinge-loss'],
    sources: ['Week1_02-PerceptronLinearClassifier.pdf', 'Production ML slides - Linear Classification Lesson 2.pdf'],
  },
  {
    id: 'empirical-risk-zero-one',
    title: 'Training Error and Zero-One Loss',
    group: 'Loss',
    week: 'Production ML W1',
    problem: 'It measures the simplest possible report card: the fraction of training examples the classifier gets wrong.',
    intuition: 'The strictest possible grader: one point off for every wrong answer, nothing for a right one, and no partial credit. Average it over the training set and you get the training error.',
    formulas: [
      { tex: tex`E_n(\theta)=\frac{1}{n}\sum_{t=1}^{n}\mathbb{1}\{y^{(t)}(\theta \cdot x^{(t)})\le 0\}`, definitions: ['E_n: training error', '1{...}: indicator equal to 1 when the condition is true', 'y^(t)(theta dot x^(t)): signed margin'] },
      { tex: tex`L_{0/1}(z)=\mathbb{1}\{z\le 0\}`, definitions: ['z: signed score y(theta dot x)', 'L_0/1: zero-one loss'] },
    ],
    example: ['Margins for four examples are [2, -0.5, 0.1, -3].', 'Mistakes have margin <= 0, so there are 2 mistakes: -0.5 and -3.', 'E_n = 2 / 4 = 0.5.'],
    graph: { type: 'zeroOne', title: 'Zero-one loss ignores distance after the threshold', sliders: [{ key: 'margin', label: 'signed margin z', min: -3, max: 3, step: 0.1, value: -0.5 }] },
    misconception: 'A margin of -0.01 and a margin of -100 both count as one mistake, which is why convex surrogate losses such as hinge become useful.',
    prerequisites: ['linear-classifier'],
    followOns: ['hinge-loss', 'model-complexity-generalization'],
    sources: ['Week1_02-PerceptronLinearClassifier.pdf', 'Week1_03-HingeLoss.pdf'],
  },
  {
    id: 'hinge-loss',
    title: 'Hinge Loss and Margins',
    group: 'Loss',
    week: 'Production ML W1',
    problem: 'It replaces the strict right-or-wrong score with one that also rewards being confidently right, which makes learning much easier.',
    intuition: 'A teacher who wants correct answers with room to spare. Wrong answers cost a lot, barely-right answers cost a little, and only confident right answers (a margin of at least 1) cost nothing.',
    formulas: [
      { tex: tex`L_{\text{hinge}}(z)=\max(0,1-z)`, definitions: ['z: signed margin y(theta dot x)', '1-z: margin shortfall', 'max: keeps loss nonnegative'] },
      { tex: tex`R_n(\theta)=\frac{1}{n}\sum_{t=1}^{n}L_{\text{hinge}}(y^{(t)}\theta \cdot x^{(t)})`, definitions: ['R_n: empirical hinge risk', ...Object.values(commonSymbols).slice(0, 4)] },
    ],
    example: ['For margins z = [1.5, 0.2, -1], losses are max(0, 1-1.5)=0, max(0, 1-0.2)=0.8, max(0, 1-(-1))=2.', 'Average hinge risk = (0 + 0.8 + 2) / 3 = 0.933.', 'The Lesson 2 slides\' examples, as (label, score): (-1, 0.4) costs max(0, 1 + 0.4) = 1.4, more than the zero-one loss of 1; (+1, -0.9) costs max(0, 1 + 0.9) = 1.9, a big penalty for a confident mistake; (+1, 0.7) costs 0.3: correct, but not yet confident.'],
    graph: { type: 'hinge', title: 'Hinge loss versus zero-one loss', caption: 'Solid: hinge loss. Dashed: zero-one loss. Hinge is never below zero-one (it is a convex upper bound on it), and it still slopes for correct points with margin below 1.', sliders: [{ key: 'margin', label: 'signed margin z', min: -3, max: 3, step: 0.1, value: 0.2 }] },
    misconception: 'Hinge loss is not the actual test error; it is a surrogate chosen because it is easier to optimize and gives richer feedback.',
    prerequisites: ['empirical-risk-zero-one', 'perceptron'],
    followOns: ['max-margin-svm', 'surrogate-losses', 'gradient-descent-method', 'ridge-regularization'],
    sources: ['Week1_03-HingeLoss.pdf', 'Production ML slides - Linear Classification Lesson 2.pdf'],
  },
  {
    id: 'stochastic-subgradient-descent',
    title: 'Stochastic Subgradient Descent',
    group: 'Optimization',
    week: 'Production ML W1-W2',
    problem: 'It makes training on huge datasets affordable by learning from one random example at a time instead of all of them.',
    intuition: 'Computing the exact downhill direction means looking at every example. Instead, step using one random example: like asking one random person for directions. Each step is noisy, but right on average and far cheaper.',
    formulas: [
      { tex: tex`\text{pick }t\text{ at random; if }y^{(t)}(\theta^{(k)}\cdot x^{(t)})\le1:\quad \theta^{(k+1)}=\theta^{(k)}+\eta_k\,y^{(t)}x^{(t)}`, definitions: [tex`\eta_k: learning rate at step k, for example 1/(k + 1); a fixed 0.1 is also popular in practice`, tex`t: randomly chosen example; examples with agreement above 1 cause no update`] },
      { tex: tex`\sum_{k=1}^{\infty}\eta_k=\infty,\quad \sum_{k=1}^{\infty}\eta_k^2<\infty`, definitions: [tex`\eta_k: step sizes that satisfy both conditions (such as 1/(k + 1)) make the method converge to the minimum of R_n (with probability 1, for convex R_n; the best-so-far R_n(theta) approaches min R_n)`] },
      { tex: tex`\theta\leftarrow\begin{cases}(1-\eta_k\lambda)\theta+\eta_k y^{(t)}x^{(t)}, & y^{(t)}(\theta\cdot x^{(t)})\le1\\ (1-\eta_k\lambda)\theta, & \text{otherwise}\end{cases}`, definitions: [tex`\lambda: with the penalty (lambda/2)||theta||^2 of the SVM objective added, every step also shrinks theta; lambda = 0 gives the rule above`] },
    ],
    example: ['Let y = +1, x = [2,1], theta = [0,0], and eta = 0.2.', 'The agreement is 0, which is at most 1, so update. (With a penalty, shrinking theta by (1 - eta lambda) would have no effect yet because theta is zero.)', 'theta becomes [0,0] + 0.2*[2,1] = [0.4,0.2].', 'Now the agreement is [0.4,0.2] . [2,1] = 1.0, still at most 1, so SSGD updates again to [0.8,0.4], even though the point is already correct. The perceptron would not.', 'The agreement is now 2 > 1, so updates on this example stop: SSGD wants confident answers, not just correct ones.'],
    graph: { type: 'ssgdHinge', title: 'SSGD on the hinge risk: keep the best theta', caption: 'The course\'s rule on eleven points that no line through the origin separates: pick a random example, update if its agreement is at most 1. The orange curve is the training risk after each step and jumps around; the blue curve is the best risk seen so far, the theta SSGD reports. Set schedule to 1 for eta_k = 1/(k + 1), which settles but moves slowly; a large constant step is jumpier.', sliders: [{ key: 'schedule', label: 'schedule (0 = constant, 1 = 1/(k + 1))', min: 0, max: 1, step: 1, value: 0 }, { key: 'eta', label: 'constant step eta', min: 0.05, max: 1.5, step: 0.05, value: 1 }, { key: 'steps', label: 'steps', min: 10, max: 300, step: 10, value: 100 }] },
    misconception: 'A stochastic training curve need not decrease every step; that is noise, not necessarily failure.',
    prerequisites: ['gradient-descent-method', 'subgradients', 'hinge-loss'],
    followOns: ['max-margin-svm', 'ridge-regularization', 'logistic-loss'],
    sources: ['Week1_03-HingeLoss.pdf', 'Production ML Slides Lesson 3 - Linear Regression.pdf'],
  },
  {
    id: 'linear-regression',
    title: 'Linear Regression',
    group: 'Model',
    week: 'Production ML W2',
    problem: 'It predicts a number, such as a price or a temperature, from input features using a straight-line relationship.',
    intuition: 'Draw the best straight line through the data, so predictions land as close as possible to the real numbers: a price from a floor area, say. With more features, the line becomes a flat plane.',
    formulas: [
      { tex: tex`f(x;\theta,\theta_0)=\theta \cdot x + \theta_0`, definitions: ['f: predicted numeric response', 'theta: feature weights', 'theta_0: intercept', 'x: feature vector'] },
      { tex: tex`\hat{y}=\theta_1x+\theta_0`, definitions: ['single-feature case of linear regression', 'theta_1: slope', 'theta_0: intercept'] },
      { tex: tex`R_n(\theta,\theta_0)=\frac1n\sum_{t=1}^{n}\frac{\bigl(y^{(t)}-\theta\cdot x^{(t)}-\theta_0\bigr)^2}{2}`, definitions: [tex`R_n: empirical risk with the squared loss Loss(z) = z^2/2 used in the Week 2 notes; the 1/2 only makes the gradient simpler`] },
      { tex: tex`\nabla_\theta R_n=-\frac1n\sum_{t=1}^{n}\bigl(y^{(t)}-\theta\cdot x^{(t)}-\theta_0\bigr)\,x^{(t)},\qquad \frac{\partial R_n}{\partial\theta_0}=-\frac1n\sum_{t=1}^{n}\bigl(y^{(t)}-\theta\cdot x^{(t)}-\theta_0\bigr)`, definitions: [tex`\nabla_\theta R_n: gradient; gradient descent repeats theta <- theta - alpha grad R_n (and the same for theta_0)`] },
      { tex: tex`\theta^{(k+1)}=\theta^{(k)}+\eta_k\bigl(y^{(t)}-\theta^{(k)}\cdot x^{(t)}\bigr)\,x^{(t)},\quad t\ \text{chosen at random}`, definitions: [tex`\eta_k: learning rate, for example 1/(k + 1); this is the stochastic gradient update from the Week 2 notes (theta_0 is absorbed by appending a constant 1 to x)`] },
      { tex: tex`L(a,b)=\frac1N\sum_{i=1}^{N}(a x_i+b-y_i)^2,\qquad \frac{\partial L}{\partial a}=-\frac2N\sum_{i=1}^{N}x_i\bigl(y_i-(a x_i+b)\bigr),\quad \frac{\partial L}{\partial b}=-\frac2N\sum_{i=1}^{N}\bigl(y_i-(a x_i+b)\bigr)`, definitions: ['L: mean squared error as written in the Lesson 3 slides, with slope a, intercept b, and N examples; it has no 1/2, so its gradients carry a factor 2, but its minimizer is the same as that of R_n', 'gradient descent: a <- a - alpha dL/da and b <- b - alpha dL/db, repeated until the changes are tiny (early stopping) or a maximum count is reached'] },
    ],
    example: ['Let theta_1 = 2 and theta_0 = 1.', 'For x = 3, prediction y_hat = 2*3 + 1 = 7.', 'If the true y is 9, residual = y - y_hat = 9 - 7 = 2.', 'Learning by gradient descent: fit y = theta x through the origin to the points (1, 2) and (2, 3). With n = 2 and the loss z^2/2, R_n(theta) = [(2 - theta)^2 + (3 - 2theta)^2]/4 and its derivative is -[(2 - theta)*1 + (3 - 2theta)*2]/2 = -(8 - 5theta)/2.', 'From theta = 0 with alpha = 0.2: the gradient is -4, so theta = 0.8; then the gradient is -2, so theta = 1.2. The steps shrink toward theta = 1.6, the same answer the normal equation gives directly.'],
    graph: { type: 'linearRegression', title: 'Line fitted to numeric responses', sliders: [{ key: 'slope', label: 'slope theta_1', min: -3, max: 5, step: 0.1, value: 1.2 }, { key: 'intercept', label: 'intercept theta_0', min: -5, max: 5, step: 0.1, value: 0.5 }] },
    misconception: 'Linear regression is linear in the parameters, not necessarily limited to raw straight-line features; transformed features can still produce curved relationships in the original input.',
    prerequisites: ['feature-representation', 'gradient-descent-method'],
    followOns: ['polynomial-regression', 'least-squares-normal-equation', 'feature-scaling', 'ridge-regularization'],
    sources: ['Week2_notes01-LinearRegression.pdf', 'Production ML Slides Lesson 3 - Linear Regression.pdf'],
  },
  {
    id: 'polynomial-regression',
    title: 'Polynomial Regression as Feature Expansion',
    group: 'Model',
    week: 'Production ML W2',
    problem: 'It fits curved relationships while still using the simple machinery of linear regression.',
    intuition: 'Want a curve, but only know how to fit lines? Add new features made from the old one: x, x^2, x^3, and so on. The model is still a weighted sum, yet as a function of x it bends.',
    formulas: [
      { tex: tex`\varphi(x)=[x,x^2,\ldots,x^K]^T`, definitions: [tex`\varphi: feature map`, tex`K: polynomial degree`] },
      { tex: tex`\hat y=b+\sum_{k=1}^{K}a_kx^k`, definitions: [tex`a_k: coefficient of x^k`, tex`b: intercept`] },
    ],
    example: ['For K = 3, use features [x, x^2, x^3].', 'At x = 2, the feature vector is [2,4,8].', 'With coefficients a = [1,0,3] and b = 5, prediction is 5 + 1*2 + 0*4 + 3*8 = 31.', 'The prediction is nonlinear in x but linear in the coefficients.'],
    graph: { type: 'generalization', title: 'Degree controls training and test error', caption: 'Illustrative curves: training error typically falls as degree rises, while test error eventually rises because high-degree curves chase noise.', sliders: [{ key: 'complexity', label: 'polynomial degree (illustrative)', min: 0, max: 10, step: 1, value: 5 }] },
    misconception: 'Polynomial regression is still linear regression with engineered features; the parameters enter linearly.',
    prerequisites: ['linear-regression', 'feature-representation'],
    followOns: ['model-complexity-generalization', 'ridge-regularization'],
    sources: ['Production ML Slides Lesson 3 - Linear Regression.pdf'],
  },
  {
    id: 'least-squares-normal-equation',
    title: 'Least Squares and the Normal Equation',
    group: 'Loss',
    week: 'Production ML W2',
    problem: 'It picks the regression line that makes the average squared prediction error as small as possible.',
    intuition: 'Measure each miss, square it, and average. The line with the smallest average is the least-squares fit, and because this loss is a smooth bowl, there is a direct formula for its bottom: the normal equation.',
    formulas: [
      { tex: tex`R_n(\theta)=\frac{1}{n}\sum_{t=1}^{n}\frac{(y^{(t)}-\theta\cdot x^{(t)})^2}{2}`, definitions: ['R_n: mean squared empirical risk (with the 1/2 of the Week 2 notes, which does not change the minimizer)', 'y^(t): true response', 'theta dot x^(t): prediction'] },
      { tex: tex`R^{\text{test}}_{n'}(\theta)=\frac{1}{n'}\sum_{t=n+1}^{n+n'}\frac{(y^{(t)}-\theta\cdot x^{(t)})^2}{2}`, definitions: [tex`R^{\text{test}}_{n'}: error on n' new examples, what we really care about; minimizing the training risk R_n is only a surrogate for it, since test data are not available when training`] },
      { tex: tex`A\hat\theta=b,\qquad A=\frac1n\sum_{t=1}^{n}x^{(t)}\bigl(x^{(t)}\bigr)^T=\frac1nX^TX,\qquad b=\frac1n\sum_{t=1}^{n}y^{(t)}x^{(t)}=\frac1nX^Ty`, definitions: [tex`A: d by d matrix; it is invertible only when the training vectors span R^d, which needs n >= d`, tex`b: d-dimensional vector; setting the gradient -b + A theta to zero gives this equation`] },
      { tex: tex`\hat{\theta}=(X^TX)^{-1}X^Ty`, definitions: ['X: design matrix', 'y: response vector', 'requires X^TX to be invertible'] },
      { tex: tex`X=\begin{bmatrix}1&x^{(1)T}\\\vdots&\vdots\\1&x^{(n)T}\end{bmatrix},\qquad \hat w=(X^TX)^{-1}X^Ty=\begin{bmatrix}\theta_0\\\theta\end{bmatrix}`, definitions: ['column of ones: lets the same formula also learn the intercept theta_0, as in the Lesson 3 slides'] },
      { tex: tex`\hat{y}=X\hat\theta=X(X^TX)^{-1}X^Ty,\qquad X^T(y-\hat{y})=0`, definitions: [tex`\hat{y}: fitted values, the orthogonal projection of y onto the column space of X`, tex`y-\hat{y}: residual vector, perpendicular to every column of X`] },
      { tex: tex`\operatorname{RMSE}=\sqrt{\frac1n\sum_{t=1}^{n}\bigl(y^{(t)}-\hat y^{(t)}\bigr)^2},\qquad \operatorname{MAE}=\frac1n\sum_{t=1}^{n}\bigl|y^{(t)}-\hat y^{(t)}\bigr|,\qquad R^2=1-\frac{\sum_t\bigl(y^{(t)}-\hat y^{(t)}\bigr)^2}{\sum_t\bigl(y^{(t)}-\bar y\bigr)^2}`, definitions: [tex`\operatorname{RMSE}: root mean squared error, a typical error size in the units of y; big misses count extra`, tex`\operatorname{MAE}: mean absolute error, also in the units of y and less affected by a few big misses`, tex`R^2: fraction of the variation of y around its mean that the model explains; 1 is perfect, 0 is no better than predicting the mean, and on test data it can be negative`] },
    ],
    example: ['Fit y = theta x through the origin to points (1,2) and (2,3).', 'X^T X = 1^2 + 2^2 = 5.', 'X^T y = 1*2 + 2*3 = 8.', 'theta_hat = 8 / 5 = 1.6. Predictions are 1.6 and 3.2.', 'Projection view: the residual y - y_hat = [2 - 1.6, 3 - 3.2] = [0.4, -0.2] is perpendicular to the column x = [1, 2]: 0.4 - 0.4 = 0.', 'Reporting the fit: RMSE = sqrt((0.4^2 + 0.2^2)/2) ≈ 0.316, MAE = (0.4 + 0.2)/2 = 0.3, and with mean y_bar = 2.5, R^2 = 1 - 0.2/0.5 = 0.6.'],
    graph: { type: 'fitLine', title: 'Least squares as visible residuals', caption: 'This graph uses a separate three-point dataset with an intercept. The red bars are residuals. Move the slope and intercept to see why the normal equation solution is the global minimum of this quadratic loss.', sliders: [{ key: 'slope', label: 'slope', min: -1, max: 2, step: 0.05, value: 1 }, { key: 'intercept', label: 'intercept', min: -1, max: 3, step: 0.05, value: 0 }] },
    figure: 'projection-plane',
    misconception: 'The normal equation is not always usable. A must be invertible, which needs the training points to span R^d (so n >= d), and inverting a d by d matrix costs about d^3 operations: with d = 10,000 that is slow, which is why the notes prefer stochastic gradient updates for large problems. The Lesson 3 slides add a third limit: most other models, such as logistic regression and neural networks, have no closed-form solution at all, which is why gradient descent is the general-purpose tool.',
    prerequisites: ['matrix-systems', 'rank-nullity', 'linear-regression', 'orthogonal-projections', 'loss-gradients'],
    followOns: ['ridge-regularization', 'multicollinearity', 'model-complexity-generalization'],
    sources: ['Week2_notes01-LinearRegression.pdf', 'Production ML Slides Lesson 3 - Linear Regression.pdf'],
  },
  {
    id: 'ridge-regularization',
    title: 'Ridge Regression and Regularization',
    group: 'Complexity',
    week: 'Production ML W2',
    problem: 'It stops a regression model from over-reacting to noise by discouraging very large weights.',
    intuition: 'Ridge attaches a gentle spring to every weight, pulling it back toward zero. The model fits the training data a little less exactly, but usually predicts new data better, because it stops chasing noise.',
    formulas: [
      { tex: tex`J_{n,\lambda}(\theta)=\frac{\lambda}{2}\lVert\theta\rVert^2+\frac{1}{n}\sum_{t=1}^{n}\frac{(y^{(t)}-\theta\cdot x^{(t)})^2}{2}`, definitions: ['J: regularized objective, as in the Week 2 notes', 'lambda: regularization strength (lambda >= 0)', '||theta||^2: sum of squared weights'] },
      { tex: tex`\theta^{(k+1)}=(1-\lambda\eta_k)\,\theta^{(k)}+\eta_k\bigl(y^{(t)}-\theta^{(k)}\cdot x^{(t)}\bigr)\,x^{(t)}`, definitions: [tex`(1-\lambda\eta_k): shrinks theta toward zero at every stochastic gradient step`] },
      { tex: tex`\Bigl(\lambda I+\tfrac1nX^TX\Bigr)\hat\theta=\tfrac1nX^Ty`, definitions: [tex`\lambda I+\tfrac1nX^TX: always invertible when lambda > 0; multiplying by n gives the formula below`] },
      { tex: tex`\hat{\theta}_{\text{ridge}}=(X^TX+n\lambda I)^{-1}X^Ty`, definitions: ['I: identity matrix', 'n lambda I: diagonal stabilizing penalty', 'scikit-learn: Ridge minimizes ||y - X w||^2 + alpha ||w||^2, so its alpha equals n lambda'] },
    ],
    example: ['For one feature through the origin, suppose X^T X = 5 and X^T y = 8.', 'With n = 2 and lambda = 0.5, denominator becomes 5 + 2*0.5 = 6.', 'theta_ridge = 8 / 6 = 1.333, smaller than least-squares theta = 1.6.'],
    graph: { type: 'ridge', title: 'Regularization shrinks the fitted slope', sliders: [{ key: 'lambda', label: 'lambda', min: 0, max: 4, step: 0.1, value: 0.8 }] },
    misconception: 'Regularization is not the same as deleting features; it softly discourages large weights while still letting useful signals matter.',
    prerequisites: ['least-squares-normal-equation', 'model-complexity-generalization'],
    followOns: ['lasso', 'train-validation-test'],
    sources: ['Week2_notes01-LinearRegression.pdf', 'Production ML Slides Lesson 3 - Linear Regression.pdf'],
  },
  {
    id: 'model-complexity-generalization',
    title: 'Model Complexity, Overfitting, and Generalization',
    group: 'Generalization',
    week: 'Production ML W1-W2',
    problem: 'It explains why the model that scores best on its training data can still predict new data badly.',
    intuition: 'A model that is too simple misses the pattern, like a student who never studied. One that is too flexible memorizes the practice questions, mistakes included, and fails the real exam. We want the one in between.',
    formulas: [
      { tex: tex`E_{\text{test}}(h)=\mathbb{E}_{(x,y)\sim P}[L(h(x),y)]`, definitions: ['E_test: expected unseen error', 'P: data-generating distribution', 'L: task loss'] },
      { tex: tex`E_{\text{train}}(h)=\frac1n\sum_{t=1}^{n}L\bigl(h(x^{(t)}),y^{(t)}\bigr)`, definitions: [tex`E_{\text{train}}: average loss on the training examples`] },
      { tex: tex`\text{gap}=E_{\text{test}}-E_{\text{train}}`, definitions: ['gap: difference between unseen and training error', 'large positive gap: common overfitting warning; a small gap with high error on both sets signals underfitting'] },
    ],
    example: ['Model A has training loss 0.40 and test loss 0.42.', 'Model B has training loss 0.05 and test loss 0.80.', 'A has gap 0.02; B has gap 0.75. Even though B fits training data better, A generalizes better.'],
    graph: { type: 'generalization', title: 'Training/test loss versus complexity', sliders: [{ key: 'complexity', label: 'model complexity', min: 0, max: 10, step: 0.1, value: 5 }] },
    misconception: 'Overfitting is not detected by training loss alone; the slides tie it to poor generalization on unseen data.',
    prerequisites: ['empirical-risk-zero-one', 'polynomial-regression'],
    followOns: ['bias-variance', 'ridge-regularization', 'train-validation-test'],
    sources: ['Week1_01-Introduction.pdf', 'Production ML Slides Lesson 3 - Linear Regression.pdf'],
  },
  {
    id: 'logistic-regression',
    title: 'Logistic Regression and Sigmoid Probabilities',
    group: 'Model',
    week: 'Production ML W2',
    problem: 'It predicts the probability of a yes/no outcome while still dividing the classes with a straight line.',
    intuition: 'A linear score can be any number, but a probability must lie between 0 and 1. The sigmoid squashes the score into that range like a dimmer switch: very negative gives almost 0, zero gives 0.5, very positive gives almost 1.',
    formulas: [
      { tex: tex`h(x)=\sigma(\theta\cdot x+\theta_0)`, definitions: ['h(x): predicted probability that y = 1 (labels here are y in {0, 1}; y = 1 plays the role of +1 in the perceptron pages)', 'sigma: sigmoid/logistic function', 'theta dot x + theta_0: linear score', 'notation: Lesson 4 writes h_theta(x) = g(theta^T x), with a constant feature x_0 = 1 standing in for theta_0'] },
      { tex: tex`\sigma(s)=\frac{e^s}{1+e^s}=\frac{1}{1+e^{-s}}`, definitions: ['s: real-valued score', 'e: base of natural logarithms'] },
      { tex: tex`\hat y=\begin{cases}1,&h(x)\ge0.5\\0,&h(x)<0.5\end{cases}`, definitions: [tex`\hat y: predicted class`, tex`h(x): predicted probability`, tex`0.5: default threshold`] },
      { tex: tex`\log\frac{p(y=1\mid x)}{p(y=0\mid x)}=\theta\cdot x+\theta_0`, definitions: ['log-odds: the log of the ratio of the two class probabilities; it is linear in x, so the boundary where it equals 0 (probability 0.5) is a hyperplane'] },
    ],
    example: ['Let theta dot x + theta_0 = 2.', 'sigma(2) = e^2 / (1 + e^2). Since e^2 ≈ 7.389, sigma(2) = 7.389 / 8.389 ≈ 0.881.', 'With a 0.5 threshold, predict class 1.', 'Lesson 4 example: theta_0 = -3, theta_1 = 1, theta_2 = 1. At (x_1, x_2) = (1, 1) the score is -3 + 1 + 1 = -1 < 0, so predict y = 0. Indeed sigma(-1) ≈ 0.269: a 26.9% chance of y = 1 and 73.1% of y = 0.', 'Curved boundary: with squared features and the score -1 + x_1^2 + x_2^2, the boundary is the unit circle. At (2, 2) the score is -1 + 4 + 4 = 7 and sigma(7) ≈ 0.999, so predict y = 1 (the chance of y = 0 is about 0.0009).'],
    graph: { type: 'sigmoid', title: 'Sigmoid maps score to probability', sliders: [{ key: 'score', label: 'score s', min: -8, max: 8, step: 0.1, value: 2 }, { key: 'steepness', label: 'steepness', min: 0.4, max: 2.5, step: 0.1, value: 1 }] },
    misconception: 'Despite its name, logistic regression is a classifier. It models the log-odds of y = 1 as a linear function of x and fits theta by maximum likelihood (minimizing cross-entropy), not by least squares; the resulting probability is then thresholded to make a class decision.',
    prerequisites: ['linear-classifier', 'linear-regression'],
    followOns: ['logistic-loss'],
    sources: ['Week2_notes02-Logistic Regression.pdf', 'Production ML Slides Lesson 4 - Logistic Regression.pdf'],
  },
  {
    id: 'logistic-loss',
    title: 'Logistic Loss and Learning',
    group: 'Loss',
    week: 'Production ML W2',
    problem: 'It is how logistic regression learns: it rewards putting high probability on the right answer and heavily punishes confident wrong answers.',
    intuition: 'If the answer is yes and the model says 80% yes, the loss is small; at 20% it is much bigger; near 0% it shoots toward infinity. Logistic loss rewards confidence only when it is right.',
    formulas: [
      { tex: tex`L(y,h)= -y\log(h)-(1-y)\log(1-h)`, definitions: ['y: binary target encoded as 0 or 1', 'h: predicted probability of y=1', 'log: natural logarithm'] },
      { tex: tex`\operatorname{Cost}(h,y)=\begin{cases}-\log h,&y=1\\-\log(1-h),&y=0\end{cases}`, definitions: ['Cost: the same loss written case by case, as in Lesson 4; the one-line form above combines both cases because y is 0 or 1'] },
      { tex: tex`J(\theta)=\frac{1}{n}\sum_{t=1}^{n}L(y^{(t)},\sigma(\theta\cdot x^{(t)}+\theta_0))`, definitions: ['J: average logistic loss', 'sigma: sigmoid', 'theta: logistic-regression weights'] },
      { tex: tex`\arg\max_\theta\prod_{t=1}^{n}p_t\ =\ \arg\max_\theta\sum_{t=1}^{n}\log p_t`, definitions: [tex`p_t: probability assigned to the observed label, p_t = h^y (1 - h)^(1 - y), so -log p_t is exactly the loss L above`, tex`\log: turns products into sums without changing the maximizer`, 'plus-minus labels: with y in {-1, +1} and score s the same loss is log(1 + e^(-y s))'] },
      { tex: tex`\nabla_\theta J=\frac1n\sum_{t=1}^{n}\bigl(h^{(t)}-y^{(t)}\bigr)\,x^{(t)},\qquad \frac{\partial J}{\partial\theta_0}=\frac1n\sum_{t=1}^{n}\bigl(h^{(t)}-y^{(t)}\bigr)`, definitions: [tex`h^{(t)}: predicted probability sigma(theta dot x^(t) + theta_0)`, tex`h^{(t)}-y^{(t)}: prediction error; learning repeats theta <- theta - alpha times this gradient`] },
    ],
    example: ['For y = 1 and predicted probability h = 0.8, loss = -log(0.8) ≈ 0.223.', 'For y = 1 and h = 0.2, loss = -log(0.2) ≈ 1.609.', 'The confident wrong-ish prediction is penalized much more: 1.609 - 0.223 = 1.386.', 'Optimizing log-likelihood is equivalent to optimizing likelihood because log is increasing, but it converts products into sums that are easier and numerically safer.', 'One gradient step: x = [1, 2] (the leading 1 is the offset), y = 1, theta = [0, 0]. Then h = sigma(0) = 0.5, the gradient is (0.5 - 1)[1, 2] = [-0.5, -1], and with alpha = 0.5, theta becomes [0.25, 0.5].'],
    graph: { type: 'logLoss', title: 'Logistic loss for true label y = 1', sliders: [{ key: 'prob', label: 'predicted probability h', min: 0.02, max: 0.98, step: 0.01, value: 0.8 }] },
    misconception: 'A probability near 0 or 1 is not automatically good; it is good only if it assigns high probability to the observed class.',
    prerequisites: ['logistic-regression', 'gradient-descent-method', 'loss-gradients', 'feature-scaling'],
    followOns: ['classification-metrics', 'taylor-hessian'],
    sources: ['Week2_notes02-Logistic Regression.pdf', 'Production ML Slides Lesson 4 - Logistic Regression.pdf'],
  },
  {
    id: 'classification-metrics',
    title: 'Confusion Matrix and Classification Metrics',
    group: 'Generalization',
    week: 'Production ML W2',
    problem: 'They show what kinds of mistakes a classifier makes, which a single accuracy number can hide, especially when one class is rare.',
    intuition: 'Accuracy alone can hide what kind of mistakes a model makes. A confusion matrix sorts every prediction into four boxes: correct yes (TP), false alarm (FP), correct no (TN), and missed yes (FN).',
    formulas: [
      { tex: tex`\operatorname{Accuracy}=\frac{TP+TN}{TP+TN+FP+FN}`, definitions: [tex`TP: true positives`, tex`TN: true negatives`, tex`FP: false positives`, tex`FN: false negatives`] },
      { tex: tex`\operatorname{Precision}=\frac{TP}{TP+FP},\quad \operatorname{Recall}=\frac{TP}{TP+FN},\quad \operatorname{Specificity}=\frac{TN}{TN+FP}`, definitions: [tex`\operatorname{Precision}: reliability of positive predictions`, tex`\operatorname{Recall}: sensitivity`, tex`\operatorname{Specificity}: true-negative rate`] },
      { tex: tex`\text{Error rate}=1-\operatorname{Accuracy}=\frac{FP+FN}{TP+TN+FP+FN}`, definitions: ['Error rate: fraction of examples classified wrongly'] },
      { tex: tex`\text{flag if }p\,C_{FN}>(1-p)\,C_{FP}\iff p>\frac{C_{FP}}{C_{FP}+C_{FN}}`, definitions: [tex`p: calibrated predicted probability of the positive class`, tex`C_{FN}, C_{FP}: costs of a missed positive and of a false alarm; the threshold is where the two expected costs are equal`] },
      { tex: tex`F_1=\frac{2\,\operatorname{Precision}\cdot\operatorname{Recall}}{\operatorname{Precision}+\operatorname{Recall}}`, definitions: [tex`F_1: harmonic mean of precision and recall, 2/(1/P + 1/R); for the first example 2/(1.25 + 2.25) ≈ 0.571`] },
    ],
    example: ['Suppose TP = 8, FP = 2, TN = 90, and FN = 10.', 'Accuracy = (8 + 90) / 110 = 0.891.', 'Precision = 8 / (8 + 2) = 0.8.', 'Recall = 8 / (8 + 10) = 0.444, so high accuracy still misses many positives.', 'A cat detector tested on 25 images (the table used in Lesson 4): TP = 11, FN = 3, FP = 2, TN = 9.', 'Accuracy = 20/25 = 0.8, so the error rate is 0.2. Precision = 11/13 ≈ 0.846, recall = 11/14 ≈ 0.786, specificity = 9/11 ≈ 0.818.'],
    graph: { type: 'threshold', title: 'Threshold changes the positive/negative trade-off', caption: 'Twenty examples sorted by predicted probability. Points right of the threshold are predicted positive. Raising the threshold usually raises precision and lowers recall.', sliders: [{ key: 'threshold', label: 'decision threshold', min: 0.05, max: 0.95, step: 0.05, value: 0.5 }] },
    misconception: 'High accuracy can be useless when positives are rare; always predicting negative can score well while recall is zero.',
    prerequisites: ['logistic-regression', 'logistic-loss', 'train-validation-test'],
    followOns: ['roc-auc', 'ml-in-production'],
    sources: ['Production ML Slides Lesson 4 - Logistic Regression.pdf'],
  },
  {
    id: 'eigenvalues-eigenvectors',
    title: 'Eigenvalues and Eigenvectors',
    group: 'Representation',
    week: 'Math W3C1',
    problem: 'They find the special directions a matrix only stretches, shrinks, or flips, without turning them.',
    intuition: 'Most arrows get knocked to a new direction when a matrix acts on them. An eigenvector is a special arrow that stays on its own line: the matrix only stretches, shrinks, or flips it.',
    formulas: [
      { tex: tex`A v = \lambda v`, definitions: ['A: square matrix', 'v: nonzero eigenvector', 'lambda: eigenvalue or stretch factor'] },
      { tex: tex`\det(A-\lambda I)=0`, definitions: ['characteristic equation used to find eigenvalues', 'I: identity matrix'] },
    ],
    example: ['Let A = [[2, 0], [0, 3]] and v = [1, 0].', 'A v = [[2,0],[0,3]][1,0]^T = [2,0]^T.', 'Because [2,0] = 2[1,0], v is an eigenvector with lambda = 2.', 'For w = [0,1], A w = [0,3] = 3w, so lambda = 3.', 'For a matrix that is not diagonal, solve the characteristic equation: for B = [[2, 1], [1, 2]], (2 - lambda)^2 - 1 = 0 gives lambda = 3 or lambda = 1.', 'Then solve (B - lambda I)v = 0: lambda = 3 gives v = [1, 1], and lambda = 1 gives v = [1, -1]. Check: B[1, 1] = [3, 3].'],
    graph: { type: 'eigen', title: 'Eigenvectors keep direction under a matrix', sliders: [{ key: 'lambda1', label: 'lambda_1', min: -3, max: 4, step: 0.25, value: 2 }, { key: 'lambda2', label: 'lambda_2', min: -3, max: 4, step: 0.25, value: 1 }, { key: 'angle', label: 'test vector angle', min: 0, max: 360, step: 5, value: 35 }] },
    misconception: 'An eigenvector cannot be the zero vector, but an eigenvalue is allowed to be zero.',
    prerequisites: ['determinant-geometry', 'linear-transformations'],
    followOns: ['trace', 'diagonalization', 'spectral-theorem'],
    sources: ['W3C1.pdf'],
  },
];

const conceptById = Object.fromEntries([...baseConcepts, ...subtopicConcepts, ...mmlConcepts, ...calculusConcepts, ...mlConcepts, ...probabilityConcepts].map((concept) => [concept.id, concept]));
export const concepts = conceptOrder.map((id) => conceptById[id]);
export const conceptMap = Object.fromEntries(concepts.map((concept) => [concept.id, concept]));

// Topics are overview pages that group subtopic concept pages; they keep the ids the concept map uses.
export { topics };
export const topicMap = Object.fromEntries(topics.map((topic) => [topic.id, topic]));
const topicOfConcept = Object.fromEntries(topics.flatMap((topic) => topic.children.map((id) => [id, topic.id])));

export function topicOf(conceptId) {
  return topicOfConcept[conceptId] ? topicMap[topicOfConcept[conceptId]] : null;
}

// Title and group of anything linkable: a concept page or a topic overview page.
export function entryFor(id) {
  return conceptMap[id] ?? topicMap[id] ?? null;
}

export const mindMapNodes = [
  { id: 'ml-workflow', x: 70, y: 250 },
  { id: 'sets-functions', x: 70, y: 390 },
  { id: 'feature-vectors', x: 245, y: 150 },
  { id: 'matrix-operations', x: 245, y: 275 },
  { id: 'matrix-multiplication-outer-product', x: 245, y: 400 },
  { id: 'matrix-systems', x: 430, y: 80 },
  { id: 'gaussian-elimination', x: 430, y: 590 },
  { id: 'solution-structure', x: 610, y: 590 },
  { id: 'vector-spaces-bases', x: 610, y: 80 },
  { id: 'linear-independence-subspaces', x: 610, y: 710 },
  { id: 'linear-transformations', x: 790, y: 80 },
  { id: 'transformation-matrix', x: 790, y: 205 },
  { id: 'composition-of-transformations', x: 970, y: 205 },
  { id: 'change-of-basis', x: 610, y: 835 },
  { id: 'invertible-transformations', x: 970, y: 335 },
  { id: 'affine-dimensionality-reduction', x: 790, y: 710 },
  { id: 'rank-inverse-determinant', x: 970, y: 80 },
  { id: 'determinants-cofactor-row-ops', x: 970, y: 720 },
  { id: 'linear-classifier', x: 430, y: 250 },
  { id: 'linear-classifier-through-origin', x: 430, y: 305 },
  { id: 'linear-separability', x: 430, y: 360 },
  { id: 'perceptron', x: 610, y: 230 },
  { id: 'perceptron-convergence', x: 610, y: 300 },
  { id: 'empirical-risk-zero-one', x: 610, y: 335 },
  { id: 'hinge-loss', x: 790, y: 300 },
  { id: 'convexity-surrogate-losses', x: 790, y: 390 },
  { id: 'gradient-descent', x: 970, y: 300 },
  { id: 'stochastic-subgradient-descent', x: 970, y: 390 },
  { id: 'linear-regression', x: 430, y: 470 },
  { id: 'polynomial-regression', x: 430, y: 710 },
  { id: 'least-squares-normal-equation', x: 610, y: 470 },
  { id: 'ridge-regularization', x: 790, y: 470 },
  { id: 'lasso-elastic-net', x: 790, y: 590 },
  { id: 'model-complexity-generalization', x: 970, y: 470 },
  { id: 'validation-cross-validation', x: 970, y: 590 },
  { id: 'logistic-regression', x: 790, y: 620 },
  { id: 'logistic-loss', x: 970, y: 620 },
  { id: 'classification-metrics', x: 970, y: 725 },
  { id: 'eigenvalues-eigenvectors', x: 250, y: 1040 },
  { id: 'diagonalization-pagerank', x: 470, y: 1040 },
  { id: 'orthogonality-spectral-theorem', x: 710, y: 1040 },
  { id: 'matrix-decompositions', x: 950, y: 1040 },
];

export const mindMapEdges = [
  ['ml-workflow', 'sets-functions'], ['sets-functions', 'feature-vectors'], ['feature-vectors', 'matrix-operations'], ['matrix-operations', 'matrix-multiplication-outer-product'], ['matrix-multiplication-outer-product', 'matrix-systems'], ['matrix-systems', 'gaussian-elimination'], ['gaussian-elimination', 'solution-structure'], ['matrix-systems', 'vector-spaces-bases'], ['vector-spaces-bases', 'linear-independence-subspaces'], ['linear-independence-subspaces', 'affine-dimensionality-reduction'], ['vector-spaces-bases', 'linear-transformations'], ['linear-transformations', 'transformation-matrix'], ['transformation-matrix', 'composition-of-transformations'], ['transformation-matrix', 'change-of-basis'], ['composition-of-transformations', 'invertible-transformations'], ['change-of-basis', 'diagonalization-pagerank'], ['invertible-transformations', 'rank-inverse-determinant'], ['linear-transformations', 'rank-inverse-determinant'], ['rank-inverse-determinant', 'determinants-cofactor-row-ops'],
  ['feature-vectors', 'linear-classifier'], ['linear-classifier', 'linear-classifier-through-origin'], ['linear-classifier-through-origin', 'linear-separability'], ['linear-separability', 'perceptron'], ['linear-classifier', 'empirical-risk-zero-one'], ['perceptron', 'perceptron-convergence'], ['perceptron-convergence', 'hinge-loss'], ['empirical-risk-zero-one', 'hinge-loss'], ['hinge-loss', 'convexity-surrogate-losses'], ['convexity-surrogate-losses', 'gradient-descent'], ['gradient-descent', 'stochastic-subgradient-descent'],
  ['feature-vectors', 'linear-regression'], ['linear-regression', 'polynomial-regression'], ['linear-regression', 'least-squares-normal-equation'], ['rank-inverse-determinant', 'least-squares-normal-equation'], ['least-squares-normal-equation', 'ridge-regularization'], ['ridge-regularization', 'lasso-elastic-net'], ['lasso-elastic-net', 'model-complexity-generalization'],
  ['model-complexity-generalization', 'validation-cross-validation'], ['validation-cross-validation', 'logistic-regression'], ['linear-classifier', 'logistic-regression'], ['logistic-regression', 'logistic-loss'], ['gradient-descent', 'logistic-loss'], ['logistic-loss', 'classification-metrics'],
  ['ml-landscape', 'ml-workflow'], ['ml-landscape', 'ml-in-production'],
  ['hinge-loss', 'max-margin-svm'], ['max-margin-svm', 'stochastic-subgradient-descent'], ['least-squares-normal-equation', 'feature-scaling'], ['feature-scaling', 'ridge-regularization'], ['feature-scaling', 'multicollinearity'], ['multicollinearity', 'ridge-regularization'], ['eigenvalues-eigenvectors', 'multicollinearity'], ['model-complexity-generalization', 'bias-variance'], ['bias-variance', 'validation-cross-validation'], ['classification-metrics', 'roc-auc'], ['roc-auc', 'ml-in-production'], ['ml-workflow', 'ml-in-production'],
  ['probability-statistics', 'logistic-loss'], ['probability-statistics', 'bias-variance'], ['probability-statistics', 'classification-metrics'], ['sets-functions', 'probability-statistics'],
  ['vector-calculus', 'gradient-descent'], ['vector-calculus', 'least-squares-normal-equation'], ['vector-calculus', 'logistic-loss'], ['vector-calculus', 'convexity-surrogate-losses'],
  ['feature-vectors', 'norms-inner-products'], ['norms-inner-products', 'orthogonality-spectral-theorem'], ['norms-inner-products', 'projections-gram-schmidt'], ['orthogonality-spectral-theorem', 'projections-gram-schmidt'], ['rank-inverse-determinant', 'projections-gram-schmidt'], ['projections-gram-schmidt', 'least-squares-normal-equation'], ['projections-gram-schmidt', 'pca'], ['eigenvalues-eigenvectors', 'trace'], ['trace', 'pca'], ['orthogonality-spectral-theorem', 'pca'], ['affine-dimensionality-reduction', 'pca'], ['matrix-decompositions', 'pca'], ['gradient-descent', 'lagrange-multipliers'], ['lagrange-multipliers', 'ridge-regularization'], ['lagrange-multipliers', 'lasso-elastic-net'],
    ['determinants-cofactor-row-ops', 'eigenvalues-eigenvectors'], ['rank-inverse-determinant', 'eigenvalues-eigenvectors'], ['affine-dimensionality-reduction', 'eigenvalues-eigenvectors'], ['logistic-loss', 'taylor-hessian'], ['eigenvalues-eigenvectors', 'diagonalization-pagerank'], ['eigenvalues-eigenvectors', 'orthogonality-spectral-theorem'], ['diagonalization-pagerank', 'matrix-decompositions'], ['orthogonality-spectral-theorem', 'matrix-decompositions'],
];
